...
def weighted_average(phase, flux, weights, window_size):
    ...
    A sliding mean (or median) without weights would be faster to calculate
    """
...
def sigma_clipper(time, flux, sigma_clip):
    ...
    print(
        'Clipping ceiling:', upper_clip,
        'values removed:', number_of_removed_values)
...
def remove_LC_data(time, flux):
    ...
    # (variation is due to Kepler's motion around the sun --> BJD converted)
    # If SC data occurs to have a random gap in the data with exactly
    ...
    for gap_multiple in range(1, multiples):
        ...
        select_SC_data = numpy.where(
            (gap_times < min_LC_gap * gap_multiple) |
            (gap_times > max_LC_gap * gap_multiple))
...
def get_lightcurve_from_file(koi_name):
    ...
    koi = numpy.genfromtxt(
        path + koi_name + '_datafile.csv',
...
def get_lightcurve_from_mast(koi_name, use_short_cadence, data_type):
    ...
    # Convert from "Kepler" BJD time to "Holczer 2016+" BJD time (offsets..grr)
...
def extract_segment(
    time, flux, mid_transit, transit_duration, segment_half_width,
        polynom_degree, make_detrending_figures, blinding,
        sigma_clip_after_detrending):
    ...
    if do_smear_correction:
        # Check smallest time between two points to determine whether LC or SC
        ...
        if smallest_gap > 25:
            ...
            print(
                'LC data,
                smear adjusted transit duration to',
                transit_duration)
        else:
    ...
    if numpy.size(outoftransit_flux_segment) > (polynom_degree + 1):
        ...
        # Combine in- and out of transit flux segments to divide both by poly
        allflux = numpy.append(
            intransit_flux_segment, outoftransit_flux_segment)
        alltime = numpy.append(
            intransit_time_segment, outoftransit_time_segment)
        detrended_flux = allflux - polynom(alltime) + 1

        # Sigma-clip outlier data again after detrending, can be more agressive
        # now. This also covers only positive values, because we want to
        # protect the actual transit. Check consistency with zero clipping!
        ...
        if make_detrending_figures:
            ...
            plt.scatter(
                intransit_time_segment,
                intransit_flux_segment,
                color='blue')
            plt.scatter(
                outoftransit_time_segment,
                outoftransit_flux_segment,
        ...
        try:
            ...
            if largest_gap < 0.05:  # unit: days. 2 LCs are ~0.042 days
                # Convert time axis (in days) to phase axis (in TDs)
                phase = (alltime - mid_transit) / transit_duration
            else:
                print(
                    'Droping transit, too large gap',
                    largest_gap * 60 * 24, 'minutes')
...
def load_master_data_catalog(filename):
    ...
    # Recommended to discard affected transits: they can offset the detrending

    return catalog


def get_koi_phasefold(
    koi_name, masterdata, sigma_clip_before_detrending,
        sigma_clip_after_detrending, use_max_ttv, make_detrending_figures,
        blinding, use_ttv, segment_half_width, polynom_degree,
        use_long_cadence, use_short_cadence, data_type):
    """Returns the phase-folded, detrended transit segments for one KOI"""

    print('Getting data...')
    # time, flux = get_lightcurve_from_file(koi_name)
    time, flux = get_lightcurve_from_mast(
        koi_name, use_short_cadence, data_type)
    ...
    for row in range(len(masterdata['koi'])):
        ...
        # We select the transits for this KOI which are not flagged (see def.)
        if masterdata['koi'][row] == koi_name and not masterdata['flag'][row]:
            ...
            # TTVs given by Holczer 2016+ as deviations from linear ephemeris
            ...
            if abs(ttv_minutes) < use_max_ttv:
                ...
                print(
                    'Too large TTV for transit at T=', Tx, 'with TTV=',
    ...
    stacked_data = numpy.column_stack(
        (this_koi_phase, this_koi_flux, this_koi_weights))
...
def make_figure(
    phase, flux, mid_window_times, mean_values, window_size,
...
def main():
    ...
    # If no SC data is available, it automatically falls back to LC data
    use_short_cadence = True
    use_long_cadence = False
    ...
    polynom_degree = 2  # 1=line, 2=parabola, etc.
    ...
    masterdata = load_master_data_catalog(path + 'masterdata.csv')
    this_koi_phase, this_koi_flux, this_koi_weights = get_koi_phasefold(
        ...
        use_short_cadence=use_short_cadence,
        data_type=data_type)
    ...
    window_size_points = int((numpy.size(this_koi_flux) /
                              (segment_half_width * 2) * window_size))
    print(
        'Window size', window_size, 'TDs, ',
        window_size_points, 'window steps')
    ...
    peak_left = get_max(
        mid_window_times, mean_values,
        start=-peak_window_start,
        end=0)
    peak_right = get_max(
        mid_window_times,
        mean_values,
        start=0,
        end=peak_window_start)
    reference_left = get_max(
        mid_window_times,
        mean_values,
        start=-segment_half_width,
        end=-peak_window_start)
    reference_right = get_max(
        mid_window_times,
        mean_values,
        start=peak_window_start,
        end=segment_half_width)
    ...
    print('Making figure...')
    make_figure(
        this_koi_phase, this_koi_flux, mid_window_times, mean_values,
        window_size, segment_half_width, blinding, polynom_degree,
        use_max_ttv, use_ttv, peak_left, reference_left, peak_right,
        reference_right)
